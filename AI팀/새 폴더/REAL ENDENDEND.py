...
# 시리얼 통신 설정
arduino = serial.Serial('COM7', 9600, timeout=1)  # COM 포트 번호와 timeout 설정

# 카메라를 연결합니다.
cap = cv2.VideoCapture(0)
cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)  # 가로 해상도를 640으로 설정
cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)  # 세로 해상도를 480으로 설정
...
while True:
    ...
    num_circles = 0
    if len(contours) == 0 and num_circles<= 3:  # 물체가 없을 경우
        ...
        for contour in contours:
            ...
            if len(approx) == 4 and cv2.isContourConvex(approx) and num_circles<= 3:
                area = cv2.contourArea(approx)
                if area > 100 and num_circles<= 3:
                    cv2.drawContours(resized, [approx], 0, (0, 255, 0), 2)

                    # 인식된 네모의 중심 좌표를 계산합니다.
                    M = cv2.moments(approx)
                    if M['m00'] != 0 and num_circles<= 3:
                        cx = int(M['m10'] / M['m00'])
                        cy = int(M['m01'] / M['m00'])
                        # 네모의 중심 좌표를 출력합니다.
            ...
            elif len(approx) >= 8 and num_circles<= 3:  # 원의 근사치 정확도를 조정 (7 -> 8)
